...
//     String strPage=null;
   ...
   String strPage = request.getParameter("pages");
    ...
    String  tename1=null;
    String teacheridcard=null;
    String state=null;
    ResultSet rs;
    String sql="select tid,tidcard,state from teacher limit "+(pages-1)*6+",6";
    rs=jdbc.executeQuery(sql);
    while(rs.next()){
    tename1=rs.getString(1);
    teacheridcard=rs.getString(2);
    state=rs.getString(3);
    if(state==null)
    {state="未审核";}
    out.println("<tr><td>"+tename1+"</td><td>"+teacheridcard+"</td><td>"+state+"</td><td><a href='checkteacher.jsp?tid="+tename1+"'>查看详情</a></td></tr>");
    }
    %>
    </table>     
  </div>   
<div id="last">
  <form name="f1" method="POST" action="admin.jsp" onSubmit="return checknum()">
            <table border="0" align="center" >
                <tr>
                    <td>第<%=pages%>页 共<%=totalpages%>页 <a href="admin.jsp?pages=1">首页</a></td>
                    <td><a href="admin.jsp?pages=<%=(pages<1)?pages:(pages-1) %>"> 上一页</a></td>
                    <td><a href="admin.jsp?pages=<%=(pages>=totalpages)?totalpages:(pages+1)%>"> 下一页</a></td>
                    <td><a href="admin.jsp?pages=<%=totalpages%>">最后一页</a></td>
